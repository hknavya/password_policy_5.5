<?php 
/**
Module Name : Search and update user
Description: Updates the existing user with additional validations for password.
	...
	public function validate_user(){
			...
			if (USER_REGISTRATION_WITH_EMAIL_ADDRESS == false) {
				...
				if (strlen($username) >= USER_USERNAME_MINIMUM && !$valc->username($username)) {
					...
					if(!empty($password)){
						/*validates password using pasword policy library*/
						PasswordPolicy::validatePassword($username,$password,$this->error);
						...
						$userID = $uo->getUserID();//get user id
						...
						/*checks if the user enters the same old password again*/
						if (in_array($encryptedPassword, $passwordList)) {
								$this->error->add(t('The Password was already used by you .please enter diffrent password'));
			...
			if($encryptedPassword!=NULL){
				...
				PasswordPolicy::savePassword($encryptedPassword,$uo->getUserID(),$uo->getUserName(),$uo->getUserEmail());
			}

...
Description: Adds a new user with the additional validations for password.
...
class DashboardUsersAddController extends Controller {
	...
	public function on_start() {
		...
		/*loads the password policy library*/
	...
	public function view(){
			...
			if ($_POST['create']) {
				...
				if (!$this->error->has()) {
					...
					$uID = $uo->getUserID(); 
					/*encrypts the password given by user*/
					$encryptedPassword = User::encryptPassword($password);
						/*saves the encrypted password into the database*/
						PasswordPolicy::savePassword($encryptedPassword,$uID,$username,$_POST['uEmail']);

...
/**
Module Name : login
Description: user login with validations.
...
class LoginController extends Controller { 
	...
	public function do_login() { 
		...
		try {
			...
			if ($u->isError()) {
				switch($u->getError()) {
					...
					case USER_INVALID:
						if (USER_REGISTRATION_WITH_EMAIL_ADDRESS) {
							...
							$passwordLoginAttempt = $pkg->config('password_login_attempt');
							...
							if($count != NULL){
								...
								$db = Loader::db();
								$db->Execute('update PasswordHistory set attempts = ? where username = ?', array($count,$this->post('uName')));
							}
							throw new Exception(t('Invalid email address or password.'));
						} else {
							/*checks the number of failed login attempts from that particular user */
							$pkg = Package::getByHandle('password_policy');
							$passwordLoginAttempt = $pkg->config('password_login_attempt');
							$data = PasswordPolicy::getAttempts($this->post('uName'));
							$count = $data['attempts'];
							...
							if($count != NULL){
								if($count >$passwordLoginAttempt){
									...
									$count=0;
									$ui = UserInfo::getByID($userID);
									/*if the failed login attempt is more than the defined number in database , user will be deactivated*/
									$ui->deactivate();
				...
				$date = date('Ymdhis',strtotime('-' . $passwordResetInterval . ' days')); 
				$getPasswordUpdatedTime = PasswordPolicy::getPasswordUpdatedTime($passwordResetInterval,$loginData['uID']);
				$updatedDate = date('Ymdhis',strtotime($getPasswordUpdatedTime[0]));
					/*validation for password repeat interval*/
					if($updatedDate < $date){
						$h = Loader::helper('validation/identifier');
						/*generate user hash*/
						$uHash = $h->generate('UserValidationHashes', 'uHash');
						$db = Loader::db();
						$db->Execute("DELETE FROM UserValidationHashes WHERE uID=?", array( $loginData['uID'] ) );	

						$db->Execute("insert into UserValidationHashes (uID, uHash, uDateGenerated, type,uDateRedeemed) values (?, ?, ?, ?,?)", array($loginData['uID'], $uHash, time(),intval(UVTYPE_CHANGE_PASSWORD),0));	
						$u->logout();
						/*redirect the user to change password page if the password date is expired*/		
						$this->redirect('login/change_password/'.$uHash.'/true');
					}
			}
	...
	// responsible for validating a user's email address
	public function change_password($uHash = '',$msgShow=false) {
		$db = Loader::db();
		if($msgShow==true){
		...
		if (is_object($ui)){
			...
			if($hashCreated < (time()-(USER_CHANGE_PASSWORD_URL_LIFETIME))) {
				...
				if(strlen($_POST['uPassword'])){
					...
					$userName = $ui->getUserName();//get username
					Loader::library('password', 'password_policy');//loads password policy library
					/*validates the password using password policy library*/
					PasswordPolicy::validatePassword($userName,$_POST['uPassword'],$e);
					$pkg = Package::getByHandle('password_policy');
					$passwordRepeatInterval = $pkg->config('password_repeat_interval');
					$userID =  $ui->getUserID();
					$encryptedPassword = User::encryptPassword($_POST['uPassword']);
					...
					$passwordCount = intval($allPasswords[0]['count(*)']);
						//validation for password expiry 
						if($passwordCount>0){
					...
					if (!$e->has()){ 
						...
						$encryptPassword = User::encryptPassword($_POST['uPassword']);
